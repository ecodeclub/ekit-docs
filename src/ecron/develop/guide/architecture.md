...
那么对于纯粹的任务本身来说，它的状态非常简单：

![任务状态图](./img/task_state.png)

- waiting：刚创建，或者说还没有被任何调度节点所调度
- scheduling：该任务已经被某个调度节点抢占了
- stopped：该任务已经被停止调度了
- deleted：该任务已经被删除，无法恢复

这些状态只是逻辑上的状态，也就是说具体的实现不一定非得有这几个值。例如说在关系型数据库的底层实现里面，deleted 本身可以是软删除的，也可以是硬删除的。

相比之下，任务执行状态图要更加复杂一点：

![任务执行状态图](./img/tast_exe_state.png)

- waiting：该状态对应于任务的 waiting 状态
- preempted：被抢占，也就是有一个调度节点已经找到了这个任务，后面将由这个节点来负责调度该任务
- runnable：到时间可以执行一次了。严格来说，这个状态才是某一次执行的起始状态。它此时可能还在等待一些资源，比如说等待目标节点空闲下来，也可能在等待一个可能的空闲执行节点。大多数情况下，它意味着任务在资源得到满足的情况下就可以被立刻投入运行
- running：运行中
- success：执行成功
- failed：执行失败

从上面也可以看出来，我们并没有详细区分重试与非重试。也就是说，单纯从状态上来看，我们看不出来某次执行是否是一次重试的执行。所以大多数时候，实习着需要在这之外引入一些必要的字段用于区分是否重试。
